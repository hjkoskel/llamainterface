...
func RunAsWebServer(portNumber int, imGen *ImageGenerator, llama *llamainterface.LLamaServer, textTemperature float64, imageTemperature float64, translator *Translator, ttsEngine *TTSInterf) error {
	webUi, errWebUi := InitWebInterface(imGen, llama, translator, ttsEngine)
...
func loadGameById(gameid string, llama *llamainterface.LLamaServer, translator *Translator, ttsEngine *TTSInterf) (AdventureGame, error) {
	...
	return loadAdventure(jsonList[0], llama, &textPromptFormatter, translator, ttsEngine)
}
...
func InitWebInterface(imGen *ImageGenerator, llama *llamainterface.LLamaServer, translator *Translator, ttsEngine *TTSInterf) (WebInterface, error) {
	...
	result.router.GET("/savegames/:gameid", func(c *gin.Context) {
		...
		game, errLoad := loadGameById(c.Param("gameid"), result.llama, translator, ttsEngine)
		if errLoad != nil {
			c.Error(errLoad)
			return
		}
	...
	result.router.GET("/newgame/", func(c *gin.Context) {
		...
		newcat, newcatErr := CreateToCatalogue(jsonFileList, llama, translator, ttsEngine)
		if newcatErr != nil {
	...
	result.router.GET("/newgame/:gamename", func(c *gin.Context) { //Latest entry on page
		...
		game, errLoad := loadAdventure(newgamefilename, result.llama, &textPromptFormatter, translator, ttsEngine)
		if errLoad != nil {
	...
	result.router.GET("/continuegame", func(c *gin.Context) { //Get catalog of all savegames
		...
		cat, catErr := CreateToCatalogue(jsonList, llama, translator, ttsEngine)
		if catErr != nil {
